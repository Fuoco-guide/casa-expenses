...
Single user version built and tested end to end, including live on Debora's iPhone over home WiFi. The Debora + Victor sync version (identity setup, Paid by, Firestore, Who Paid card) is built and tested locally in fallback mode (`FIREBASE_READY = false`), but **not yet live**, `FIREBASE_CONFIG` in `app.js` still holds placeholder values. Waiting on Debora to create the Firebase project and send the real config plus confirm the security rules are published, see "Two people, one house budget" above for the exact steps she was given. Not yet permanently hosted either, currently only reachable through this Mac's local server. Once Firebase is live, permanent hosting becomes more important than before (two phones both need a stable URL, not just one on home WiFi).

## How to make updates
1. Edit the files directly in `~/CLAUDE/CASA-EXPENSES/`
2. Test locally: `cd ~/CLAUDE/CASA-EXPENSES && python3 -m http.server 8743`, then open `http://localhost:8743/index.html`. With `FIREBASE_READY` false this exercises everything except real cross device sync.
3. To test on a phone over WiFi: find this Mac's IP with `ipconfig getifaddr en0`, then open `http://<that IP>:8743/index.html` in Safari on the phone
4. After editing `app.js`, `styles.css`, or `index.html`, bump `CACHE_NAME` in `sw.js` (e.g. v3 to v4). Otherwise the service worker keeps serving the old cached version to anyone who already added the app to their home screen, this bit us once already during testing.
5. Once permanently hosted, an update just means pushing the changed files to whatever static host she picks
6. To test the real Firebase sync end to end: open the app in two separate browser profiles or devices, set one to Debora and one to Victor, add an expense on one, confirm it appears on the other without a manual reload
